...
function formatComplaintLocation(complaint) {
  ...
  if (!hasCoords) return { community: complaint?.community || '', lat: '', lng: '', mapsLink: '' };
...
function getProcessingTimeCategory(complaint) {
  const isCompleted = complaint?.status === 'completed' || complaint?.status === 'ดำเนินการเสร็จสิ้น';
  if (!isCompleted) return 'ยังไม่เสร็จสิ้น';
...
export default function ExportComplaints({
  ...
  const convertToCSV = (data) => {
    ...
    const headers = ['ลำดับ','สถานะ','หมวดหมู่','ปัญหาที่พบ','หัวข้อ','ชื่อผู้แจ้ง','เบอร์โทร','ชุมชน','ละติจูด','ลองจิจูด','ลิงก์แผนที่','วันที่สร้าง','อัปเดตล่าสุด','มอบหมายแล้ว','วันที่มอบหมาย','หมายเหตุ','วิธีการแก้ไข','วันที่เสร็จสิ้น','ช่วงระยะเวลาดำเนินการ'];
    const csvData = data.map((complaint, index) => {
      ...
      return [
        index + 1, complaint.status || '', complaint.category || '', formatProblemsFound(complaint),
        complaint.detail || '', complaint.fullName || '', complaint.phone || '', community, lat, lng, mapsLink,
    ...
    return [headers.join(','), ...csvData.map(row => row.map(csvCell).join(','))].join('\n');
  };
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (!csvContent) { alert('ไม่มีข้อมูลให้ export'); return; }
      const filename = getFilename ? getFilename() : `complaints_${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const handleExportFiltered = async (status) => {
    ...
    try {
      ...
      if (!csvContent) { alert('ไม่มีข้อมูลให้ export'); return; }
      const statusText = { 'รอการมอบหมาย': 'pending', 'อยู่ระหว่างดำเนินการ': 'in_progress', 'ดำเนินการเสร็จสิ้น': 'completed' }[status] || 'all';
  ...
  if (allOnly) {
    return (
      <button type="button" onClick={handleExport} disabled={exporting || isDisabled} className={buttonClassName} title={isDisabled ? 'ไม่มีข้อมูลในช่วงที่เลือก' : `ส่งออก ${complaints.length} เรื่อง`}>
        ...
        {buttonLabel}
        {!exporting && complaints.length > 0 && <span className="text-xs opacity-70">({complaints.length})</span>}
      </button>
    );
  }

  const exportOptions = [
    { id: 'all', label: 'Export ทั้งหมด', icon: '📊', count: complaints.length, onClick: handleExport, disabled: complaints.length === 0 },
    { id: 'pending', label: 'Export รอการมอบหมาย', icon: '⏳', count: countByStatus('รอการมอบหมาย'), onClick: () => handleExportFiltered('รอการมอบหมาย'), disabled: countByStatus('รอการมอบหมาย') === 0 },
    { id: 'in_progress', label: 'Export กำลังดำเนินการ', icon: '🔄', count: countByStatus('อยู่ระหว่างดำเนินการ'), onClick: () => handleExportFiltered('อยู่ระหว่างดำเนินการ'), disabled: countByStatus('อยู่ระหว่างดำเนินการ') === 0 },
    { id: 'completed', label: 'Export เสร็จสิ้น', icon: '✅', count: countByStatus('ดำเนินการเสร็จสิ้น'), onClick: () => handleExportFiltered('ดำเนินการเสร็จสิ้น'), disabled: countByStatus('ดำเนินการเสร็จสิ้น') === 0 },
  ];

  return (
    <div className="dropdown dropdown-end">
      <div tabIndex={0} role="button" className="btn btn-sm btn-ghost gap-2" disabled={exporting}>
        {exporting ? <span className="loading loading-spinner loading-xs" /> : (
      ...
      <ul tabIndex={0} className="dropdown-content menu z-[100] mt-2 w-72 rounded-xl border border-gray-200 bg-white p-2 shadow-lg">
        ...
        {exportOptions.map((option) => (
          <li key={option.id}>
            <button type="button" onClick={option.onClick} disabled={exporting || option.disabled} className="flex w-full items-center justify-between gap-2 rounded-lg disabled:opacity-40">
              <span className="flex items-center gap-2"><span>{option.icon}</span><span>{option.label}</span></span>
        ...
        <li className="pointer-events-none mt-1 border-t border-gray-100 px-3 pt-2">
          <span className="text-[11px] leading-snug text-gray-400">ไฟล์ CSV รูปแบบ UTF-8 พร้อม BOM รองรับภาษาไทย</span>
        </li>
      </ul>
    </div>
  );
