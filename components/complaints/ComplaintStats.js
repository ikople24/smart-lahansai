...
function ProgressBar({ value, max, color }) {
  ...
  return (
    <div className="h-1.5 bg-gray-200 rounded-full overflow-hidden">
      <div className={`h-full transition-all duration-1000 ease-out ${color}`} style={{ width: `${percentage}%` }} />
    </div>
  );
}
    ...
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
      {statCards.map((card, index) => (
        <div
          ...
          className={`${card.color} border rounded-xl p-4 cursor-pointer hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all duration-300 ease-out`}
        ...
        >
          ...
          <div className="mt-3">
            <ProgressBar value={card.value} max={stats?.total || 1} color={card.progressColor} />
          </div>
        </div>
      ))}
    </div>
