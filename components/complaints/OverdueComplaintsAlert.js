...
export default function OverdueComplaintsAlert({
  complaints,
  assignments,
  onComplaintClick,
  ...
  useEffect(() => {
    ...
    const overdue = complaints
      ...
      .map(complaint => {
        ...
        return { ...complaint, daysSinceUpdate, isAssigned: !!assignment };
      })
  ...
  if (overdueComplaints.length === 0) return null;
      ...
      >
        ...
        <div className="flex items-center gap-2">
          <div className="hidden sm:flex items-center gap-1.5">
            {criticalCount > 0 && (
              <span className="px-2 py-0.5 bg-red-100 text-red-700 text-xs font-medium rounded-full">วิกฤต {criticalCount}</span>
            )}
            {warningCount > 0 && (
              <span className="px-2 py-0.5 bg-orange-100 text-orange-700 text-xs font-medium rounded-full">เร่งด่วน {warningCount}</span>
            )}
            {infoCount > 0 && (
              <span className="px-2 py-0.5 bg-amber-100 text-amber-700 text-xs font-medium rounded-full">ติดตาม {infoCount}</span>
            )}
          </div>
      ...
      <div className={`transition-all duration-300 ease-out ${isExpanded ? 'max-h-[400px]' : 'max-h-48'}`}>
          ...
          {displayedComplaints.map((complaint) => {
            ...
            return (
              <div
                ...
                className="group flex items-center gap-3 p-2.5 bg-white rounded-lg border border-gray-100 hover:border-blue-200 hover:bg-blue-50/50 cursor-pointer transition-all duration-200"
              >
                <div className={`w-1.5 h-10 rounded-full ${severity.level === 'critical' ? 'bg-red-500' : severity.level === 'warning' ? 'bg-orange-500' : 'bg-amber-400'}`} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className={`text-xs font-medium px-1.5 py-0.5 rounded ${severity.level === 'critical' ? 'bg-red-100 text-red-700' : severity.level === 'warning' ? 'bg-orange-100 text-orange-700' : 'bg-amber-100 text-amber-700'}`}>
                  ...
                  <p className="text-sm font-medium text-gray-800 truncate group-hover:text-blue-600 transition-colors">{complaint.detail}</p>
                  <p className="text-xs text-gray-400 truncate">{complaint.category} • {new Date(complaint.updatedAt).toLocaleDateString('th-TH')}</p>
                </div>
        ...
        {overdueComplaints.length > 3 && !isExpanded && (
          <div className="px-4 pb-3">
            <p className="text-xs text-center text-gray-500">และอีก {overdueComplaints.length - 3} เรื่อง...</p>
          </div>
        )}
      </div>
    </div>
